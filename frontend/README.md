...
schema width, not dataset size, so cost stays flat whether the upload has 500 rows
or 500,000.

**Data-minimization by design**
Raw row data never leaves the database to reach the LLM. Every stage, profiling,
semantic inference, planning, is built on statistical aggregates: column stats,
value counts, correlations. The model sees the shape of the data, not the data
itself.
